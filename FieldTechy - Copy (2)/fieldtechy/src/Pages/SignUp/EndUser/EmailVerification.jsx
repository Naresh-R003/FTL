...
import { EmailContext } from '../../../EmailContext'; // Assuming EmailContext is set up

const EmailVerification = () => {
  const [otp, setOtp] = useState(new Array(6).fill(""));
  const [timeLeft, setTimeLeft] = useState(60);
  const [isResendEnabled, setIsResendEnabled] = useState(false);
  const { email } = useContext(EmailContext); // Using context to get email
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://192.168.1.18:8080/auth/verify-email?email=${encodeURIComponent(email)}&otp=${enteredOtp}`, {}, {
        headers: {
  ...
  const handleResendCode = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.1.18:8080/auth/verify-email', {
        email
      }, {
        headers: {
          'Content-Type': 'application/json',
  ...
  return (
    <div className='outer'>
      <div className='otpContainer'>
        ...
        <div>
          <h1 className='otpHeader'>Two-step verification</h1>
        </div>
        <div className='otpPara'>
          Thanks for registering verify email
        </div>
        <div>
          <p className='otpPara'>For security purposes, please enter the OTP we have sent to your email {email}</p>
        </div>
